...
!		====		==========			=====================
!    05/28/2015    Bicheng Chen			The assignment represents the default values
!    06/08/2016    Bicheng Chen         Move all variable to new param.f90
!    10/20/2016    Bicheng Chen         Add dynamically wind stress and wave condition
!
...
namelist/output_control/ output, use_avgslice, base_time, average_dim_num, &
                         ...
                         flagVSpl, nVSpl, flagCSpl, nCSpl, flag_srfV, tt_srfV
logical :: output = .true.
...
logical :: flagVSpl = .false.       ! the flag to output the velocity in specific areas
integer :: nVSpl = int_missing      ! the number of specific area to output the velocity field
logical :: flagCSpl = .false.       ! the flag to output the velocity (concentration? cyan) in specific areas
integer :: nCSpl = int_missing      ! the number of specific area to output the concentration field
logical :: flag_srfV = .false.      ! the flag to output surface velocity field
integer :: tt_srfV = int_missing    ! the time steps to output sruface velocity field

namelist/output_sample/  xVSplS, xVSplE, yVSplS, yVSplE, zVSplS, zVSplE, &
                         ttVSpl, xCSplS, xCSplE, yCSplS, yCSplE, zCSplS, zCSplE, ttCSpl
integer, dimension(:), allocatable :: xVSplS, yVSplS, zVSplS    ! the lower bound of ouput areas in x,y,z-direction
integer, dimension(:), allocatable :: xVSplE, yVSplE, zVSplE    ! the upper bound of ouput areas in x,y,z-direction
integer, dimension(:), allocatable :: xCSplS, yCSplS, zCSplS
integer, dimension(:), allocatable :: xCSplE, yCSplE, zCSplE
integer :: ttVSpl                   ! the time steps to output velocity in specific areas
integer :: ttCSpl                   ! the time steps to output concentration in specific areas
...
namelist/flow_param/ u_star, u_scale, Pr, restart_vel, inilag, interp, DYN_init,       &
                     ...
                     inflow, jx_s, x_relax, jx_p, x_relaxp,             &
...
integer :: jx_s = int_missing, x_relax = int_missing
integer :: jx_p = int_missing, x_relaxp = int_missing
real(rprec) :: face_avg = 1._rprec
...
! --- canopy parameters
namelist/canopy_param/ use_trees, use_res_canopy, canopy_type,      &
                       use_field_scale, use_plant_scale, hc, hgap,  &
                       lad, Cd_const, vel_exp 
logical :: use_trees = .false.
logical :: use_res_canopy = .true.
