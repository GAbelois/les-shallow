...
! if (.not. ocean_flag) then
    if ( coordz == 0 ) call obukhov()
! end if
...
call mpi_sendrecv(p(1, 1, nz - 1), ldx*nynpy, MPI_RPREC, up,   2, &
                  ...
                  comm, status, ierr)
call mpi_sendrecv(p(1, 1, 1),  ldx*nynpy, MPI_RPREC, down, 3, &
                  p(1, 1, nz), ldx*nynpy, MPI_RPREC, up,   3, &
                  comm, status, ierr)
