...
!real(kind=rprec) , dimension(ld,nyt,nz) :: visc,dissip
!real(kind=rprec), dimension(ld,nyt,nz) :: Betaclip
!real(kind=rprec), dimension(ld,nyt,nz) :: Cs_opt2_2d,Cs_opt2_4d
!real(kind=rprec), dimension(ld,nyt) :: S,tempos
...
! if ( rank == 0 ) then
!     write(fname, '(a,i8.8,a)') '../output/FLM_check_', jt, '.csv'
!     open(9,file=fname)
!     write(9, '(9a)') 'F_LM', ',', 'F_MM', ',', 'F_QN', ',', 'F_NN'
!     do jz = 1, nz-1
!     do jy = 1, nynpy
!     do jx = 1, nxt
!         write(9, fmt) F_LM(jx, jy, jz), ',', F_MM(jx, jy, jz), ',', &
!                       F_QN(jx, jy, jz), ',', F_NN(jx, jy, jz)
!     end do
!     end do
!     end do
!     close(9)
...
!--why set ldx-1:ldx to this?
    ...
    !TS FIX THE BUG IN COMPAQ COMPILER
    !TS ADD the following
    Tn = max(real(F_QN(:, :, jz)*F_NN(:, :, jz)), real(1.e-24))
    Tn = opftdelta*(Tn**powcoeff)
    Tn(:, :) = max(real(1.e-24), real(Tn(:, :)))
    dumfac = lagran_dt/Tn
    epsi = dumfac/(1._rprec + dumfac)
...
!   Cs_opt2_4d(:,:,jz) = SUM(QN(:,:))/SUM(NN(:,:))
    Cs_opt2_4d(ldx, :, jz) = 1.e-24
    Cs_opt2_4d(ldx - 1, :, jz) = 1.e-24
    Cs_opt2_4d(:, :, jz) = max(real(1.e-24), real(Cs_opt2_4d(:, :, jz)))
    ...
    Cs_opt2(ldx, :, jz) = 1.e-24
    Cs_opt2(ldx - 1, :, jz) = 1.e-24
    Cs_opt2(:, :, jz) = max(real(1.e-24), real(Cs_opt2(:, :, jz)))
...
!print *,'jt,jz,beta,betaclip,Cs_opt2',jt,jz,sum(Beta(1:nxt,1:nyt,jz))/(nxt*nyt),&
!sum(Betaclip(1:nxt,1:nyt,jz))/(nxt*nyt),sum(Cs_opt2(1:nxt,1:nyt,jz))/(nxt*nyt)
!print *,'jt,jz,Cs_4,Cs_2,Cs',jt,jz,sum(Cs_opt2_4d(1:nxt,1:nyt,jz))/(nxt*nyt),&
!sum(Cs_opt2_2d(1:nxt,1:nyt,jz))/(nxt*nyt),sum(Cs_opt2(1:nxt,1:nyt,jz))/(nxt*nyt)
!print *,'jt,jz,Cs_4/Cs_2,Cs_2/Cs',jt,jz,sum(Cs_opt2_4d(1:nxt,1:nyt,jz))/&
!sum(Cs_opt2_2d(1:nxt,1:nyt,jz)),sum(Cs_opt2_2d(1:nxt,1:nyt,jz))/sum(Cs_opt2(1:nxt,1:nyt,jz))

!if (mod(jt,100) == 0) then
!      if (jz==1.or.jz==nz/4.or.jz==nz/2) then
!         write(91)real(jt*dt),real(jz),real(Cs_opt2_2d(1:nxt,1:nyt,jz))
!         write(92)real(jt*dt),real(jz),real(Cs_opt2_4d(1:nxt,1:nyt,jz))
!      end if
!end if
