...
!!!!!!!--------------------------------------------
!!real(kind=rprec),dimension(ldx,nyt,nz):: scalar
!!real(kind=rprec),dimension(ldx,nyt,nz):: theta,q ! theta and q specified in sim_param

!!real(kind=rprec),dimension(ldx,nyt,nz):: dsdx,dsdy,dsdz
    ...
    if (ubc==1 .and. damping_method==2) then !add the damping term to the scalar equation
        do jz = 1, nz-1
            RHS_T(1:nxt,1:nynpy,jz) = RHS_T(1:nxt,1:nynpy,jz)-0.5_rprec*(sponge(jz)+sponge(jz+1))*&
                            (theta(1:nxt,1:nynpy,jz)-sum(theta(1:nxt,1:nynpy,jz))/(nxt*nynpy))
        end do
    end if
...
!-----------------------------------------------------------------------     
    ...
    real(kind=rprec), dimension(nxt2, ny2npy, 0:nz) :: u_m,v_m,w_m,dsdx_m,dsdy_m,dsdz_m
...
!           if (jt .eq. hetero_array_freqz) then !If Block 200.1 STARTS
    ...
    do jz = 0, nz
        call zero_padding(u(:,:,jz), u_m(:,:,jz))
        call zero_padding(v(:,:,jz), v_m(:,:,jz))
        call zero_padding(w(:,:,jz), w_m(:,:,jz))
...
! xxxxx --------- MPI Comment block ends ------------------XXXX
    ...
    !     do j = 1, ny2npy
    !     do i = 1, nxt2
    !         RHS_m(i,j,nz-1) = (u_m(i,j,nz-1)+ust(nz-1))*dsdx_m(i,j,nz-1)    &
    !                         + (v_m(i,j,nz-1)+vst(nz-1))*dsdy_m(i,j,nz-1)    &
    !                         + (0.5_rprec*w_m(i,j,nz-1))*dsdz_m(i,j,nz-1)
    !     end do
    !     end do
    ! end if
...
!-----------------------------------------------------------------------
    ...
    call calc_hor_avg3(phi_m(1:nxt,1:nynpy), phi_m_tmp)
    call calc_hor_avg3(psi_m(1:nxt,1:nynpy), psi_m_tmp)
    call calc_hor_avg3(phi_h(1:nxt,1:nynpy), phi_h_tmp)
    call calc_hor_avg3(psi_h(1:nxt,1:nynpy), psi_h_tmp)
    call calc_hor_avg3(T_s(1:nxt,1:nynpy), T_s_tmp)

    !-------------------- OUTPUT ------------------------------
    ! if ( ver_rank == 0 ) then
    !     write(*,100) L_*z_i, ustar_*u_scale, theta_*T_scale,    &
    !                  T_s_tmp*T_scale, dz/2/L_, wt_*u_scale*T_scale
        
    !     ! Output the heat flux time series to a file to be used later
    !     open(47,file=path//'result/WT_sfc_tseries.out',status="unknown",position="append")
    !     write(47,'(2(E14.5))') (nums+1)*dt, wt_
    !     close(47)
    ! end if 


    if (((restart_theta) .and. ((nums+1) .ge. theta_init_time)) .or. &
        ((.not. restart_theta) .and. ((nums+1) .ge. theta_init_time+1))) then

        fr = 1._rprec/c_count
        obuk_L = obuk_L + fr*L_
        obuk_ustar = obuk_ustar + fr*ustar_
        obuk_wstar = obuk_wstar + fr*wstar_avg
        obuk_phi_m = obuk_phi_m + fr*phi_m_tmp
        obuk_psi_m = obuk_psi_m + fr*psi_m_tmp
        obuk_phi_h = obuk_phi_h + fr*phi_h_tmp
        obuk_psi_h = obuk_psi_h + fr*psi_h_tmp
        ...
        if (mod(jt,c_count)==0) then
            ...
            !     write(47,fmt_5168) (nums+1)*dt,obuk_L,obuk_ustar,obuk_wstar,obuk_phi_m,obuk_psi_m,&
...
100 format('L(m),ustar(m/s),theta_1(K),T_s(K),z/L,wt_s(Km/s):',(5(1x,E13.6),1x,E13.6))    
    ! ---
...
!-----------------------------------------------------------------------
    ...
    ! if (flag_2Dsrc .and. (.not. pointsource) .and. &
    !     (jt >= ini_src .and. jt < end_src)) then
    !     do ipcon = 1, npcon
    !         do ind = 1, con_src(ipcon)%n
    !             if ( rank == con_src(ipcon)%icpu(ind) ) then
    !                 if (jt == 1) then
    !                     inquire(iolength=len_dcon) con_src(ipcon)%con_2D
    !                     write(fn_dcon, "(A,I0,A)") './inflow/inflowCon_s', ipcon, '.out'
    !                     open(unit=fid_dcon, file=fn_dcon, access='direct', recl=len_dcon)
    !                     read(unit=fid_dcon, rec=1) con_src(ipcon)%con_2D
    !                     read(unit=fid_dcon, rec=2) con_src(ipcon)%dcon_2D
    !                     close(fid_dcon)
    !                     con_src(ipcon)%con_2D =&
    !                         con_src(ipcon)%con_2D - con_src(ipcon)%dcon_2D
    !                 else if (mod(jt, tt_2Dsrc)==1) then
    !                     write(fn_dcon, "(A,I0,A)") './inflow/inflowCon_s', ipcon, '.out'
    !                     rec_dcon = jt/tt_2Dsrc + 2
    !                     open(unit=fid_dcon, file=fn_dcon, access='direct', recl=len_dcon)
    !                     read(unit=fid_dcon, rec=rec_dcon) con_src(ipcon)%dcon_2D
    !                     close(fid_dcon)
    !                 end if
    !                 con_src(ipcon)%con_2D =&
    !                     con_src(ipcon)%con_2D + con_src(ipcon)%dcon_2D
    !                 PCon(1:nxt, 1:nynpy, con_src(ipcon)%iz_cpu(ind), ipcon) =&
    !                     con_src(ipcon)%con_2D
    !             end if
    !         end do
    !     end do
    ...
    call PCon_RHS_calc2(PCon,dPCondz,PCon_sfc_temp,zo_PCon,RHS_PCon,sgs_PCon3,res_PCon3,sfc_flux,sgs_PCon1)
...
!-----------------------------------------------------------------------
    ...
    if (pointsource .and. (jt >= ini_src)) then
        do ipcon = 1, npcon
            released = 1.5_rprec*((jt-ini_src+1)*dt&
                                *sum(con_src(ipcon)%rls)*dx*dy*dz) &
                     - 0.5_rprec*((jt-ini_src)*dt&
                                *sum(con_src(ipcon)%rls)*dx*dy*dz)
        end do
    end if
    
    if (pointsource .and. (jt >= end_src)) THEN
        do ipcon = 1, npcon
    ...
    if ( rank == 0 ) then
        ...
        write(1,'(I8,6G18.8)') jt, (released), (airborne_global), &                          
                               (deposited_global), (gone_global), &                                     
...
!   
!-----------------------------------------------------------------------
    subroutine PCon_RHS_calc2(scalar,dsdz,S_Surf,z_os,RHS,sgs_vert,  &
...
!DY Changed by Di Yang for pconsgs_acc_flag=.true.
    ...
    integer :: i, j, k, jz, ipcon, ind, jx, jy, jz_min
    ! ---
...
!-----------------------------------------------------------------------
    ...
    do i = 1, (nynpy-1)
        do j = 1, i
            matrix_y(i,j) = -(nynpy-i)*(1._rprec/nynpy)
        end do
    
        do j = (i+1), nynpy
            matrix_y(i,j) = i*(1._rprec/nynpy)
        end do
    end do
    matrix_y(nynpy,:) = 1._rprec/nynpy

    ! Calculate v-component of velocity (Loop over all xz-locations)
    do i = 1, nxt
    do k = 1, nz
        ! Assemble derivative vector
        dvector_y(1:nynpy-1) = dy*dvdy(i,1:nynpy-1,k)
        dvector_y(nynpy) = sum(v(i,1:nynpy,k))
    ...
    ! Use periodicity
    temp1(:,:) = v_int0(:,1,1:nz)
    call mpi_sendrecv(temp1(:,:),  ldx*nz, mpi_rprec, south, 1,    &
                      temp0m(:,:), ldx*nz, mpi_rprec, north, 1,    &
                      comm, status, ierr)
    v_int0(:,nynpy+1,1:nz) = temp0m(:,:)
    ! v_int0(1:nxt,nynpy+1,1:nz) = v_int0(1:nxt,1,1:nz)
    ! call mpi_sendrecv(v_int0(:,1,1:nz),       ldx*nz, mpi_rprec, south, 1,    &
    !                   v_int0(:,nynpy+1,1:nz), ldx*nz, mpi_rprec, north, 1,    &
    !                   comm, status, ierr)
...
!-----------------------------------------------------------------------
    ...
    ! ---
    integer, intent(in) :: ipcon  
    real(kind=rprec), dimension(ldx,0:nynpy+1,0:nz,npcon), intent(out) :: RHS 
    integer :: i, j, k
...
!-----------------------------------------------------------------------
    ...
    real(kind=rprec), dimension(ldx,0:nynpy+1,0:nz,npcon), intent(in) :: scalar
    real(kind=rprec), dimension(ldx,0:nynpy+1,0:nz,npcon), intent(out) :: RHS 
    real(kind=rprec), dimension(ldx,0:nynpy+1,0:nz), intent(out) :: sgs_vert                  ! Store SGS vertical pollen flux
    real(kind=rprec), dimension(ldx,0:nynpy+1,0:nz), intent(out) :: sgs_x 
    integer :: i, j, k
...
!-----------------------------------------------------------------------
    ...
    do i = 1, nxt
            ...
            if ( coordy == 0 ) then
                ...
                ! gradient ratio
                    ry=(scalar(i,2,k,ipcon)-scalar(i,1,k,ipcon))&
                        /(scalar(i,1,k,ipcon)-scalar(i,0,k,ipcon))
                ! bound the gradient
                    ry=max(0._rprec,min(min(2._rprec*ry,0.75_rprec*ry&
                        +0.25_rprec),4._rprec))
                ! intrepolate the value
                    scalar_y(i,2,k)=scalar(i,1,k,ipcon)&
                        +ry*(scalar(i,1,k,ipcon)-scalar(i,0,k,ipcon))/2._rprec
                end if
            end if
        else
        ! to avoid problems of divisions by zero
        ...
        if (v_int(i,nynpy,k)+vst(k) >= 0._rprec) then
            ...
            ! to avoid problems of divisions by zero
            if ((scalar(i,nynpy,k,ipcon)-scalar(i,nynpy+1,k,ipcon))==0._rprec) then
                scalar_y(i,nynpy,k) = scalar(i,nynpy,k,ipcon)
            else
            ! gradient ratio
                ry=(scalar(i,nynpy-1,k,ipcon)-scalar(i,nynpy,k,ipcon))&
                    /(scalar(i,nynpy,k,ipcon)-scalar(i,nynpy+1,k,ipcon))
            ! bound the gradient
                ry=max(0._rprec,min(min(2._rprec*ry,0.75_rprec*ry&
                    +0.25_rprec),4._rprec))
            ! intrepolate the value
                scalar_y(i,nynpy,k)=scalar(i,nynpy,k,ipcon)&
                    +ry*(scalar(i,nynpy,k,ipcon)-scalar(i,nynpy+1,k,ipcon))/2._rprec
            end if
        end if
    end do
    end do
