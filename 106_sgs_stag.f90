...
! call mpi_sendrecv(Nu_t(1, 1, 1),  ldx*nynpy, MPI_RPREC, down, 1,  &
!                   Nu_t(1, 1, nz), ldx*nynpy, MPI_RPREC, up,   1,  &
!                   comm, status, ierr)
! call mpi_sendrecv(dissip(1, 1, 1),  ldx*nynpy, MPI_RPREC, down, 2,  &
!                   dissip(1, 1, nz), ldx*nynpy, MPI_RPREC, up,   2,  &
!                   comm, status, ierr)
! call mpi_sendrecv(magS(1, 1, 1),  ldx*nynpy, MPI_RPREC, down, 3,  &
!                   magS(1, 1, nz), ldx*nynpy, MPI_RPREC, up,   3,  &
!                   comm, status, ierr)
! ---
!if ( coordz == 0 ) then
!    write(fname, '(a,i8.8,a)') '../output/Nut_check_', jt, '.csv'
!    open(9,file=fname)
!    write(9, '(7a)') 'Nu_t', ',', 'S', ',', 'Cs_opt2'
!    do jz = 1, nz-1
!    do jy = 1, nyt
!    do jx = 1, nxt
!        write(9, fmt) Nu_t(jx, jy, jz), ',', S(jx, jy), ',', Cs_opt2(jx, jy, jz)
!    end do
!    end do
!    end do
!    close(9)
