...
real(kind=rprec), dimension(nxhnpy, nyt, 0:nz) :: a, b, c
...
if ( coordz == 0 ) then
    !if ((jt .gt. theta_init_time) .and. (remote_flag .eq. 1)) then
    ...
    ! Pressure bc modification at the surface for surf temp bc
        ...
        surf_temp_pr_modi(1:nxt, :) = g*(z_i/(u_scale**2))*(T_s - sum(T_s)/real(nxt*nynpy))
...
! set oddballs to 0, \cyan already taken care of in fftn_mpi
! probably can get rid of this if we're more careful below
! if ( coordy == npy - 1 ) then
!     H_x(nxhnpy, :, 1:nz - 1) = 0._rprec
!     H_y(nxhnpy, :, 1:nz - 1) = 0._rprec
!     H_z(nxhnpy, :, 1:nz - 1) = 0._rprec
! end if
! H_x(:, nyt/2 + 1, 1:nz - 1) = 0._rprec
! H_y(:, nyt/2 + 1, 1:nz - 1) = 0._rprec
! H_z(:, nyt/2 + 1, 1:nz - 1) = 0._rprec
! !--with MPI; topw and bottomw are only on top & bottom processes
! if ( coordy == npy - 1 ) then
!     topw(nxhnpy, :) = 0._rprec
!     bottomw(nxhnpy, :) = 0._rprec
! end if
! topw(:, nyt/2 + 1) = 0._rprec
! bottomw(:, nyt/2 + 1) = 0._rprec
...
if ( coordz == npz - 1 ) then
    ...
    if (TRI_DEBUG) then
        ...
        b(:, :, nz) = 2._rprec
        c(:, :, nz) = BOGUS !--was 0._rprec
        RHS_col(:, :, nz) = real(nz + coordz*(nz - 1), rprec)
    end if
    jz_max = nz - 1
else 
    jz_max = nz
...
do jy = 1, nyt
    ...
    do jx = 1, nxhnpy - 1
        ...
        b(jx, jy, jz) = -(kx_2d_mpi(jx, jy)**2 + ky_2d_mpi(jx, jy)**2 + 2._rprec/(dz**2))
        ...
        if (TRI_DEBUG) then
            ...
            b(jx, jy, jz) = 2._rprec
            c(jx, jy, jz) = 1._rprec
            RHS_col(jx, jy, jz) = jz + coordz*(nz - 1)
        end if
    end do
end do
end do
