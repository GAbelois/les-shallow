...
!     use param, only : nyt, nz, pi, nsteps, jt
...
!     real (rprec) :: wgt
!     real (rprec) :: u_tmp(nyt, 0:nz), v_tmp(nyt, 0:nz), w_tmp(nyt, 1:nz+1)
...
!         write (88, '(3(a,i0))') 'zone, f=point, i= ', nyt, ', j= ', nz,  &
!            ', k= ', nsteps
!       else
...
!    implicit none
!    ! ---
!    nwrite = base_time ! TOMAS CHOR included base_time in param.f90
...
!-----------------------------------------------------------------------
    ...
    real (rprec), dimension(ldx, nyt, 1:nzt) :: u_tot
    real (rprec), dimension(ldx, nyt, 1:nzt) :: v_tot
    real (rprec), dimension(ldx, nyt, 1:nzt) :: w_tot
    real (rprec), dimension(ldx, nyt, 1:nzt) :: theta_tot
    real (rprec), dimension(ldx, nyt, 1:nzt, npcon) :: PCon_tot
    real (rprec), dimension(ldx, nyt, 1:nzt) :: Cs_opt2_tot
...
!-----------------------------------------------------------------------
  ...
  if ((USE_MPI .and. coordz == 0) .or. (.not. USE_MPI)) then !LV1
    !-BC generate the file name
    write(fname, '(a,i8.8,a,i8.8,a)')&
       path//'output/vel_srf_tt', jt_total_init,&
       '_tt', jt_total_init+nsteps,&
       '.out'
    print *,fname

    !-BC output the data
    if (jt==tt_srfV) then !LV2
      open(1, file = fname, form='unformatted', status='unknown')
    else
      open(1, file = fname, form='unformatted', status='old', position='append')
    end if !LV2
    write(1) u(1:nxt, 1:nyt, 1), v(1:nxt, 1:nyt, 1), w(1:nxt, 1:nyt, 1)
    close(1)

  endif !LV1

    end subroutine check_srfV    
!-----------------------------------------------------------------------
!   
!-----------------------------------------------------------------------    
...
!-----------------------------------------------------------------------
    ...
    real, dimension(ldx, nyt, 1:nzt) :: u_tot
    real, dimension(ldx, nyt, 1:nzt) :: v_tot
    real, dimension(ldx, nyt, 1:nzt) :: w_tot
    real, dimension(ldx, nyt, 1:nzt) :: RHSx_tot
    real, dimension(ldx, nyt, 1:nzt) :: RHSy_tot
    real, dimension(ldx, nyt, 1:nzt) :: RHSz_tot
    real, dimension(ldx, nyt, 1:nzt) :: theta_tot
    real, dimension(ldx, nyt, 1:nzt, npcon) :: PCon_tot
    real, dimension(ldx,nyt,nzt) :: Cs_opt2_tot
    real, dimension(ldx,nyt,nzt) :: F_LM_tot
    real, dimension(ldx,nyt,nzt) :: F_MM_tot
    real, dimension(ldx,nyt,nzt) :: F_QN_tot
    real, dimension(ldx,nyt,nzt) :: F_NN_tot
    real, dimension(ldx,nyt,nzt) :: RHS_T_tot
    real, dimension(ldx,nyt,nzt,npcon) :: RHS_PCon_tot
    real, dimension(ldx,nyt,nzt) :: Kc_t_tot
    real, dimension(nxt,nyt,nzt) :: F_KX_tot
    real, dimension(nxt,nyt,nzt) :: F_XX_tot
    real, dimension(nxt,nyt,nzt) :: F_KX2_tot
    real, dimension(nxt,nyt,nzt) :: F_XX2_tot
    ...
    call mpi_sendrecv(RHSx(1, 1, 1),  ldx*nynpy, MPI_RPREC, down, 1, &
                      RHSx(1, 1, nz), ldx*nynpy, MPI_RPREC, up,   1, &
                      comm, status, ierr)
    call mpi_sendrecv(RHSy(1, 1, 1),  ldx*nynpy, MPI_RPREC, down, 2, &
                      RHSy(1, 1, nz), ldx*nynpy, MPI_RPREC, up,   2, &
                      comm, status, ierr)
    call mpi_sendrecv(RHSz(1, 1, 1),  ldx*nynpy, MPI_RPREC, down, 3, &
                      RHSz(1, 1, nz), ldx*nynpy, MPI_RPREC, up,   3, &
                      comm, status, ierr)
    ! ---
